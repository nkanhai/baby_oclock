...
def parse_voice_input(transcript):
    ...
    # Parse amount (look for number + ounce/oz)
    amount_match = re.search(r'(\d+(?:\.\d+)?)\s*(?:ounce|oz)', lower)
    if amount_match:
        ...
        # Check for "X and a half" patterns
        for word, num in number_words.items():
            if f'{word} and a half' in lower or f'{word} and half' in lower:
                result['amount'] = num + 0.5
                break

        # Check for single number words
        if result['amount'] is None:
            for word, num in number_words.items():
                pattern = rf'\b{word}\s+(?:ounce|oz)'
...
def format_parsed_result(parsed):
    ...
    if parsed['amount'] is not None:
        parts.append(f"Amount: {parsed['amount']} oz")

    if parsed['duration'] is not None:
        parts.append(f"Duration: {parsed['duration']} min")
